...
public class RtfSink extends SinkAdapter
{
    ...
    public void tableRows_()
    {
        ...
        for ( int i = 0; i < table.rows.size(); ++i )
        {
            ...
            if ( table.grid )
            {
                if ( i == ( table.rows.size() - 1 ) ) bb = true;
                br = false;
            }
            ...
            for ( int j = 0, x = x0; j < table.numColumns; ++j )
            {
                if ( table.grid )
                {
                    if ( j == ( table.numColumns - 1 ) ) br = true;
    ...
    {
        if ( bt ) writer.println( "\\clbrdrt\\brdrs\\brdrw" + BORDER_WIDTH );
        if ( bb ) writer.println( "\\clbrdrb\\brdrs\\brdrw" + BORDER_WIDTH );
        if ( bl ) writer.println( "\\clbrdrl\\brdrs\\brdrw" + BORDER_WIDTH );
        if ( br ) writer.println( "\\clbrdrr\\brdrs\\brdrw" + BORDER_WIDTH );
    }
    ...
    {
        if ( paragraph == null ) beginParagraph( new Paragraph() );
    }
    ...
    private void beginParagraph( Paragraph paragraph )
    {
        ...
        if ( style != STYLE_ROMAN ) beginStyle( style );
    }
    ...
    private void writeImage( String source ) throws Exception
    {
        ...
        else
        {
            ...
            if ( imageCompression )
            {
                ...
                dib.bitmap =
                    new byte[bitmap.length + ( 2 * ( bitmap.length / 255 + 1 ) )];
                dib.biSizeImage =
                    WMFWriter.Dib.rlEncode8( bitmap, 0, bitmap.length,
                                             dib.bitmap, 0 );
            }
            else
            {
    ...
    public void horizontalRule()
    {
        ...
        if ( skip > 0 ) writer.print( "\\sb" + skip );
        space.setNext( skip );
    ...
    private class Paragraph
    {
        ...
        {
            writer.print( "\\pard" );
            if ( style > 0 ) writer.print( "\\s" + style );
            switch ( justification )
            {
                case AptParser.JUSTIFY_LEFT:
                default:
            ...
            if ( leftIndent != 0 ) writer.print( "\\li" + leftIndent );
            if ( rightIndent != 0 ) writer.print( "\\ri" + rightIndent );
            if ( firstLineIndent != 0 ) writer.print( "\\fi" + firstLineIndent );
            if ( spaceBefore != 0 ) writer.print( "\\sb" + spaceBefore );
            if ( spaceAfter != 0 ) writer.print( "\\sa" + spaceAfter );

            if ( frame ) writer.print( "\\box\\brdrs\\brdrw" + BORDER_WIDTH );
    ...
    private class Space
    {
        ...
        {
            writer.print( "\\pard" );
            if ( ( space -= 10 ) > 0 ) writer.print( "\\sb" + space );
            writer.println( "\\plain\\fs1\\par" );
        }
    ...
    private class Table
    {
        ...
        int width()
        {
            ...
            if ( grid ) width += ( numColumns + 1 ) * BORDER_WIDTH;
            return width;
        }
