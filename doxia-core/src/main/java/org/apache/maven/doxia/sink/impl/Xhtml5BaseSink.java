...
public class Xhtml5BaseSink extends AbstractXmlSink implements HtmlMarkup {
    ...
    public void table_() {
        ...
        if (this.tableContentWriterStack.isEmpty()) {
            LOGGER.warn("{}No table content", getDocumentLocator().getLogPrefix());
            return;
        }
    ...
    public void anchor(String name, SinkEventAttributes attributes) {
        ...
        if (!DoxiaUtils.isValidId(id)) {
            ...
            LOGGER.debug(
                    "{}Modified invalid anchor name '{}' to '{}'",
    ...
    public void unknown(String name, Object[] requiredParams, SinkEventAttributes attributes) {
        if (requiredParams == null || !(requiredParams[0] instanceof Integer)) {
            LOGGER.warn(
                    "{}No type information for unknown event '{}', ignoring!",
                    getDocumentLocator().getLogPrefix(),
                    name);
        ...
        if (tag == null) {
            LOGGER.warn(
                    "[]No HTML tag found for unknown event '{}', ignoring!",
                    getDocumentLocator().getLogPrefix(),
                    name);
        } else {
