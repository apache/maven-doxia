...
public class Xhtml5BaseParserTest extends AbstractParserTest {
    ...
    public void testSemanticTags() throws Exception {
        ...
        assertEquals("inline", event.getName());
        assertEquals(
                "semantics=rubyTextContainer", event.getArgs()[0].toString().trim());
        ...
        assertEquals("inline", event.getName());
        assertEquals(
                "semantics=bidirectionalIsolation",
                event.getArgs()[0].toString().trim());

        event = it.next();
        assertEquals("inline", event.getName());
        assertEquals(
                "semantics=bidirectionalOverride", event.getArgs()[0].toString().trim());
